...
public class RendererTeleportBeacon extends TileEntitySpecialRenderer<TileEntityTeleportBeacon>
{
                       ...
                       float alpha)
    {
        ...
        IBakedModel model = Minecraft.getMinecraft().getRenderItem()
                .getItemModelWithOverrides(TileEntityTeleportBeacon.TOPPER_ITEM_WHEN_ACTIVE, te.getWorld(), null);
        model = ForgeHooksClient.handleCameraTransforms(model, ItemCameraTransforms.TransformType.GROUND, false);

        Minecraft.getMinecraft().getTextureManager().bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
        Minecraft.getMinecraft().getRenderItem().renderItem(TileEntityTeleportBeacon.TOPPER_ITEM_WHEN_ACTIVE, model);
